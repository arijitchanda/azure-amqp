...
public class Util implements Constants {
	...
	public static SSLContext getContext(com.AzureIoTHub.azureamqp.sampler.AbstractAzureIoTHubSampler sampler) throws Exception {
		if (!sampler.isDualSSLAuth()) {
			...
			try (InputStream is_cacert = new FileInputStream(theFile1);
				...
				SSLContext sslContext = SSLContexts.custom().loadTrustMaterial(tks, new TrustSelfSignedStrategy()) // use
																													...
																													// to
																													// customize
						.loadKeyMaterial(cks, CLIENTCERT_PASS.toCharArray()) // load
																				// client
																				// certificate
						.build();
				return sslContext;
